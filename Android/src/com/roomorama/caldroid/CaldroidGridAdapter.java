...
public class CaldroidGridAdapter extends BaseAdapter {
	...
	protected void customizeTextView(int position, TextView cellView) {
		...
		if ((minDateTime != null && dateTime.lt(minDateTime))
			...
			if (CaldroidFragment.disabledBackgroundDrawable == -1) {
				cellView.setBackgroundResource(R.drawable.caldroid_disable_cell);
			} else {
				cellView.setBackgroundResource(CaldroidFragment.disabledBackgroundDrawable);
			}

			if (dateTime.equals(getToday())) {
				cellView.setBackgroundResource(R.drawable.caldroid_red_border_gray_bg);
			}
		} else {
		...
		if (shouldResetDiabledView && shouldResetSelectedView) {
			// Customize for today
			if (dateTime.equals(getToday())) {
				cellView.setBackgroundResource(R.drawable.caldroid_red_border);
			} else {
				cellView.setBackgroundResource(R.drawable.caldroid_cell_bg);
			}
		}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// For reuse
		if (convertView == null) {
			cellView = (TextView) inflater.inflate(R.layout.caldroid_date_cell, null);
		}
